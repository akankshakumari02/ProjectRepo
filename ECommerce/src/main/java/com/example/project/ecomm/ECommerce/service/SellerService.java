...
public class SellerService
{
    ...
    public void addProduct(ProductDto productDto, String email) {
        ...
        if (!category.isPresent()) {
            throw new FieldNotFoundException("Category Id not found.");
        }
    ...
    public Product getProduct(Integer id, String email)
    {
        ...
        {
            throw new ProductEntityException("This Product is not Listed by This Seller");
        }
        return product1;
    }
    ...
    public void updateProduct(ProductDto productDto, Integer id) {
        ...
        productOptional.orElseThrow(() -> new FieldNotFoundException("Product  not found"));
    ...
    public void addProductVariation(ProductVariationDto productVariationDto, String email){
        ...
        productOptional.orElseThrow(() -> new FieldNotFoundException("No product found"));
        Product product = productOptional.get();
        if (!(product.getSeller().getId()==seller.getId())|| !product.isIs_Active() || product.isIs_Deleted()) {
            throw new FieldNotFoundException("No product found");
        }
    ...
    private void validate(Category category, List<MetadataDto> metadataFieldValues) {
        ...
        for (MetadataDto metadataDto : metadataFieldValues) {
            ...
            if (!categoryMetadataFieldIds.contains(metadataFieldId)) {
                throw new FieldNotFoundException("Category Metadata Field not found"); //if category metadatafield is associated with it or not
            }
            ...
            if (result.isEmpty()) {
                throw new FieldNotFoundException("Metadata Field Value should be within the possible values");
            }
        }
    }
    ...
    public ProductVariation getProductVariationById(String email, Integer productVariationId)
    {
        ...
        productVariationOptional.orElseThrow(() -> new FieldNotFoundException("No product variation found"));
        ...
        throw new FieldNotFoundException("No product variation found");
    }
    ...
    public List<OrderProduct> viewAllOrders(String email) {
        List<Product> productList = getAllProducts(email);
        List<ProductVariation> productVariationList = new ArrayList<>();
        for (Product product : productList) {
            productVariationList.addAll(getAllProductVariationForProductById(email, product.getId()));
        }

        List<OrderProduct> orderProduct = new ArrayList<>();
        for (ProductVariation productVariation : productVariationList) {
            orderProduct.addAll(orderProductRepository.findByProductVariationId(productVariation.getId()));
        }
        return orderProduct;
    }


    public void changeOrderStatus(StatusDto statusDto, Principal principal)
    {
        ...
        if (!orderProduct1.getProductVariation().getProduct().getSeller().equals(seller))
            throw new FieldNotFoundException("Order Product does not belong to this seller");
