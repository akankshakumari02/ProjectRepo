...
public class SellerService
{
    ...
    public void addProductVariation(ProductVariationDto productVariationDto, String email){
        ...
        List<MetadataDto>  metadataFieldIdValues= productVariationDto.getMetadataDto();
    ...
    private void validate(Category category, List<MetadataDto> metadataFieldIdValues) {
        Integer categoryId = category.getId();
        List<Integer> categoryMFIds = category.getCategoryMetadataFieldValuesList()
                .stream()
        ...
        for (MetadataDto metadataDto : metadataFieldIdValues) {
            int metadataFieldId = metadataDto.getId();
            if (!categoryMFIds.contains(metadataFieldId)) {
                throw new RuntimeException("Category Metadata Field not found");
            }
    ...
    private String buildMetadata(List<MetadataDto> metadataFieldIdValues) {
        ...
        for (var mv : metadataFieldIdValues) {
            CategoryMetadataField categoryMetadataField = categoryMetadataFieldRepository.findById(mv.getId()).get();
            sb.append("\"")
    ...
    public void updateProductVariation(UpdateProductVariationDto updateProductVariationDto, String email) {
        Integer productVariationId = updateProductVariationDto.getProductVariationId();
        Seller seller = (Seller) userService.findUserByEmail(email);
        Optional<ProductVariation> productVariationOptional = productVariationRepository.findById(productVariationId);
        productVariationOptional.orElseThrow(() -> new RuntimeException("No product variation found"));
        ProductVariation productVariation = productVariationOptional.get();
        ...
        Category category = product.getCategory();
        List<MetadataDto> metadataFieldIdValues = updateProductVariationDto.getUpdateMetadata();
        validate(category, metadataFieldIdValues);
        ModelMapper mm = new ModelMapper();
